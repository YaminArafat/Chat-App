...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget checkIfFriend(String email) {
    ...
    return DialogButton(
        ...
        onPressed: () {
          firebaseFirestore.collection('${loggedInUser.email}').add({
            'Friend Email': email,
          });
        },
        child: Row(
  ...
  StreamBuilder<QuerySnapshot<Map<String, dynamic>>> findFriendsStream() {
          ...
          if (snapshot.hasData) {
            try {
              for (var user in snapshot.data!.docs) {
                if (user.data()['Email'] != loggedInUser.email) {
                  ...
                  for (var friend in curUserFriends!.docs) {
                    if (user.data()['Email'] == friend.data()['Friend Email']) {
                      setState(() {
            ...
            return Center(
              child: Text(
                ...
                style: TextStyle(
                    color: Colors.blue, fontFamily: 'Ubuntu', fontSize: 20),
              ),
            );
          } else {
            return Center(
